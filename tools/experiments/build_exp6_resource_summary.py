        ...
        rows.append(
            {
                ...
                **gpu,
                ...
                "path_scope": "DECAL-exclusive detect-to-stitch components",
...
def write_md(rows: list[dict[str, str]], path: Path, csv_path: Path) -> None:
    ...
    lines = [
        ...
        "",
        "These numbers come from rerunning the same 300-frame `decal_server` samples under `/usr/bin/time -v` while sampling `nvidia-smi`.",
        "",
    ...
    lines += [
        "",
        "## DECAL-Exclusive Processing Cost",
        "",
    ...
    lines += [
        ...
        "",
        "A 900-frame FM6 run (`eval/exp6/resource_curve/fm6_00_900f_w1/resource_curve.md`) records CPU/RSS and GPU telemetry over elapsed time. The average CPU load is 142.1%, while GPU utilization averages 4.3%, reinforcing that the optimized scaffold is CPU/memory and benchmark-overhead limited rather than GPU-inference limited.",
        "",
        "## Notes",
        "",
        "- `Avg path` is the sum of DECAL-specific measured components from server-side detection through client-side stitching; it is not an end-to-end pipeline time.",
        "- Common baseline work (video decode, display/frame presentation, and ordinary encode/transport) was not measured in this trace and is reported as N/A rather than folded into DECAL modules.",
