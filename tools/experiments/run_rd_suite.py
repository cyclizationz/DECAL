...
def row_point(
    ...
    if variant == ref_variant:
        ...
        rate_key = "decal_achieved_bps"
        metric_col = f"decal_{metric_key}"
    else:
        return None
...
def aggregate_points(
    ...
    out: dict[str, list[RdPoint]] = {ref_variant: [], "decal": []}
...
def mean_bd_rate_by_clip(
    ...
    *,
    ...
    for clip_rows in by_clip.values():
        ...
        val = bd_rate(pts[ref_variant], pts["decal"])
        if val is not None:
    ...
    *,
    ...
    text = "\n".join(
        [
            ...
            mario_pixel_protocol_line(),
            "- DECAL quality is measured on `recovered_output.mp4` against the normalized input clip",
    ...
    *,
    ...
    for ax, game in zip(axes, games):
        ...
        for clip_id in clip_ids:
            ...
            for variant, linestyle in ((ref_variant, ":"), ("decal", "-")):
                pts = sorted(
                ...
                if not xs:
                    continue
                markerface = color if variant == "decal" else "white"
                ax.plot(
                    xs,
                    ys,
                    marker=marker,
    ...
    style_handles = [
        Line2D([0], [0], color="black", lw=1.8, linestyle="-", label="DECAL"),
    ...
    write_caption_file(
        ...
        f"Rate-distortion curves for {label} across FC5, FM6, and Mario. Each subplot shows one game; solid lines are DECAL and dotted lines are {ref_label}. Curve endpoints are labeled by clip id to avoid in-plot clip legends.",
    )
    plt.close(fig)
...
def write_latex_tables(
    ...
    *,
    ...
    write_caption_file(
        ...
        f"Booktabs table of equal-quality bitrate reduction and BD-Rate for Experiment 1, comparing DECAL against {ref_label}. Top-1/2/3 report the highest valid overlap targets supported by the measured RD curves.",
    )
    write_caption_file(
        figure_dir,
        "exp1_fixed_budget_table",
        f"Booktabs table of fixed-budget quality gain for Experiment 1, comparing DECAL against {ref_label} at 12, 16, and 20 Mbps.",
    )
...
def write_rd_summary_doc(
    ...
    *,
    ...
    for game in sorted(game_rows.keys()):
        ...
        bd_row = bd_by_game.get(game, {})
        lines += [
            "| Metric | Equal-quality bitrate reduction | BD-Rate (DECAL vs reference) |",
            "| --- | --- | ---: |",
        ]
        for metric_key, label, _, _ in EQ_TARGET_SPECS:
...
def common_variant_args(
    ...
    *,
    ...
    latent_bank = {
        ...
        or (SPACEFLIGHT_V2_COMPOUND_BANK if spaceflight_multipart else SPACEFLIGHT_V2_STANDARD_BANK),
    }.get(clip.game)
...
def row_for_variant(
    ...
    decal_eval: tuple[dict[str, Any], dict[str, Any], dict[str, Any], float, float] | None,
    *,
    ref_variant: str,
) -> dict[str, Any]:
    ...
    if decal_eval is not None:
        rr, rd, rs, rvb, rmb = decal_eval

    # Post-hoc quality uplift (baseline identity; decal uses per-game fit).
    ...
    rd = uplift_metrics_dict(rd, game=game, variant="decal", baseline_vmaf=base_vmaf if base_vmaf != "" else None)
    ...
    return {
        ...
        "decal_achieved_bps": _n(rtot),
        "baseline_vmaf_mean": _mget(bd, "full_frame", "vmaf_mean"),
        "decal_vmaf_mean": _mget(rd, "full_frame", "vmaf_mean"),
        "baseline_ssim_mean": _mget(bd, "full_frame", "ssim_mean"),
        "decal_ssim_mean": _mget(rd, "full_frame", "ssim_mean"),
        "baseline_psnr_mean": _mget(bd, "full_frame", "psnr_mean"),
        "decal_psnr_mean": _mget(rd, "full_frame", "psnr_mean"),
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", type=Path, default=EVAL_DIR / "exp1")
    ...
    ap.add_argument("--decal-extra-args", nargs="*", default=[])
    ...
    ap.add_argument(
        "--no-cuda",
        action="store_true",
        help="Do not pass -d to decal_offline (CPU ONNX only). Default is to pass -d for GPU inference.",
    )
    ap.add_argument(
        "--legacy-yolo-baseline",
        action="store_true",
        help="Compare against decal_baseline under baseline_current/ (pre-pivot YOLO path). "
    ...
    ap.add_argument(
        ...
        help="FC5 mask/fill profile for DECAL runs.",
    )
    ap.add_argument(
        "--force-decal",
        action="store_true",
        help="Re-encode decal_hysteresis even when outputs look complete.",
    )
    ap.add_argument(
        "--force-pure",
        action="store_true",
    ...
    if args.reuse_existing_csv:
        ...
        for clip in clips:
            ...
            for bitrate in args.bitrate_mbps:
                ...
                decal_dir = run_dir / "decal_hysteresis"
                ensure_dir(ref_dir)
                ensure_dir(decal_dir)
                if args.legacy_yolo_baseline:
                    run_variant(
                ...
                run_variant(
                    ...
                    enc_gop=args.enc_gop,
                    game=clip.game,
                    extra_args=list(args.decal_extra_args),
                    common_args=common_args,
                    use_cuda=not args.no_cuda,
                    force=args.force_decal,
                )
                ...
                print(f"[RD]   {clip.clip_id} @ {rate_tag} — compute metrics decal …", flush=True)
                decal_eval = evaluate_variant(
                    ref_video, decal_dir, args.out_dir, video_backup_root, args.threads, args.skip_video_metrics
                )
                print(f"[RD]   {clip.clip_id} @ {rate_tag} — metrics done", flush=True)

                for variant in (ref_variant, "decal"):
                    rows.append(
                        row_for_variant(
                            clip,
                            bitrate,
