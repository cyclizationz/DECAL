...
from common import CURRENT_BASELINE_BIN, DATA_ROOT, DEFAULT_MODEL, EVAL_DIR, FC5_LATENT_BANK, FC5_MODEL, FM6_LATENT_BANK, FM6_MODEL, MARIO_TEMPLATES, OFFLINE_BIN, SPACEFLIGHT_LATENT_BANK, SPACEFLIGHT_MODEL, ensure_dir, load_manifest, safe_name
from pixel_mario_defaults import mario_pixel_args
from video_metrics import compute_video_metrics


PIXEL_TEMPLATES = DATA_ROOT / "templates" / "mario_kinds"
PIXEL_SINGLE_TEMPLATE = MARIO_TEMPLATES
    ...
    *,
    ...
    if clip.game == "spaceflight":
        ...
        if spaceflight_multipart:
            base += ["--multipart-object", "--multipart-class", "0"]
        return base + ["--latent-bank", str(bank)]
    return base


def learned_ablation_configs(
    clip: Any,
    *,
    spaceflight_multipart: bool = False,
    spaceflight_latent_bank: Path | None = None,
) -> list[dict[str, Any]]:
    base = learned_base_args(
        clip,
    ...
    for feather in (0, 4, 8, 16):
        configs.append(
            {
                "name": f"fill_dominant_feather_{feather}",
                "args": base + [
                    "--yolo-matcher",
                    "latent_key",
                    "--mask-color",
...
def plot_ablation(rows: list[dict[str, Any]], out_dir: Path) -> None:
    if plt is None:
        return
    learned = [r for r in rows if r.get("status") == "ok" and r.get("game") in {"fc5", "fm6", "spaceflight"}]
    if learned:
        ...
        if fill_rows:
            fig, ax = plt.subplots(figsize=(7.5, 4.5))
            ax.scatter(
                [float(r.get("decal_vmaf_mean", 0.0) or 0.0) for r in fill_rows],
                [float(r.get("bsp_pct", 0.0) or 0.0) for r in fill_rows],
            )
            for row in fill_rows:
                ax.annotate(str(row["config_name"]), (float(row.get("decal_vmaf_mean", 0.0) or 0.0), float(row.get("bsp_pct", 0.0) or 0.0)), fontsize=7)
...
def main() -> None:
    ...
    ap.add_argument("--out-dir", type=Path, default=EVAL_DIR / "exp4")
    ...
    for clip in clips:
        clip_configs = (
            mario_ablation_configs()
            if clip.game == "mario"
            else learned_ablation_configs(
                clip,
        ...
        for cfg in clip_configs:
            ...
            decal_bps = delivered_bps(run_dir)
            ref_bps = reference_bps(run_dir)
            bsp = (1.0 - decal_bps / ref_bps) * 100.0 if ref_bps > 1e-9 else None
            row = {
                "clip_id": clip.clip_id,
                "game": clip.game,
                "config_name": cfg["name"],
            ...
            if args.compute_video_metrics:
                ...
                row["decal_vmaf_mean"] = metrics["full_frame"]["vmaf_mean"]
