...
CASES = [
    {
        "game": "FC5",
        "clip_id": "fc5_00",
        "input": str(DATA_ROOT / "clips" / "fc5_00.mp4"),
        "model": str(DATA_ROOT / "models" / "fc5_seg.onnx"),
        "extra": ["--latent-key", "--cache-mode", "partial-warm"],
    },
    {
        "game": "FM6",
        "clip_id": "fm6_00",
        "input": str(DATA_ROOT / "clips" / "fm6_00.mp4"),
        "model": str(DATA_ROOT / "models" / "fm6_seg.onnx"),
...
def run_case(case: dict[str, object], workers: int, depth: int, max_frames: int) -> dict[str, object]:
    ...
    cmd = [
        str(REPO / "deployment" / "build" / "decal_server"),
        "--input",
