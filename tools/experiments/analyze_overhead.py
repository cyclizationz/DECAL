...
from common import EVAL_DIR, ensure_dir, infer_template_bytes, load_manifest, seconds_to_frames
...
def simulate_cache(
    *,
    ...
    for frame_idx, payload in enumerate(payloads):
        ...
        decal_frame_bytes = float(decal_bytes[frame_idx] if frame_idx < len(decal_bytes) else 0)
        cumulative_baseline += baseline_frame_bytes
        cumulative_decal += decal_frame_bytes
        cumulative_decal += float(msk1_bytes_per_frame[frame_idx] if frame_idx < len(msk1_bytes_per_frame) else 0)
        ...
        if frame_forced_raw:
            forced_raw += 1
            if synthetic_config is not None and synthetic_config.enabled:
                penalty = max(0.0, baseline_frame_bytes - decal_frame_bytes)
                forced_raw_penalty_bytes += penalty
                cumulative_decal += penalty
        ...
        net_saved_cumulative.append(cumulative_baseline - cumulative_decal)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Analyze template/control overhead for offline DECAL runs.")
    ap.add_argument("--rd-dir", type=Path, default=EVAL_DIR / "exp1")
    ...
    ap.add_argument("--out-dir", type=Path, default=EVAL_DIR / "exp2")
    ...
    for (clip_id, rate_point), variants in sorted(pair_rows.items()):
        ...
        if ref_key not in variants or "decal" not in variants:
            continue
        run_dir = Path(variants["decal"])
        ...
        decal_per_frame_csv = run_dir / "per_frame_metrics.csv"
        if baseline_per_frame_csv.exists() and decal_per_frame_csv.exists():
            with baseline_per_frame_csv.open("r", newline="") as fb, decal_per_frame_csv.open("r", newline="") as fr:
                rb = list(csv.DictReader(fb))
                rr = list(csv.DictReader(fr))
                n = min(len(rb), len(rr))
                for i in range(n):
                    baseline_bytes.append(int(float(rb[i].get("masked_bytes", 0) or 0)))
                    decal_bytes.append(int(float(rr[i].get("masked_bytes", 0) or 0)))
