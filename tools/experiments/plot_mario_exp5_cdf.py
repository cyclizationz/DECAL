...
`rd_suite_points.csv` (after merge), clips mario_00..mario_04, decal variant only.
Writes figure under eval/exp5/.
"""
...
EXP1 = REPO / "eval" / "exp1_tuned_upper_bound_shortclips"
FORMER_CSV = EXP1 / "rd_suite_tuned_upper_bound_shortclips_points.csv"
GRID_CSV = EXP1 / "rd_suite_points.csv"
OUT_DIR = REPO / "eval" / "exp5"
CLIP_IDS = tuple(f"mario_0{i}" for i in range(5))


def load_decal_rows(path: Path) -> list[dict[str, str]]:
    with path.open(newline="") as f:
        rows = list(csv.DictReader(f))
    keep = {c for c in CLIP_IDS}
    return [r for r in rows if r.get("clip_id") in keep and r.get("variant") == "decal"]
...
def main() -> None:
    ...
    former = load_decal_rows(FORMER_CSV)
    grid = load_decal_rows(GRID_CSV)
    if len(former) != 25 or len(grid) != 25:
        print(
            f"warning: expected 25 decal rows per arm, got former={len(former)} grid={len(grid)}",
            flush=True,
        )
    ...
    lines = [
        "Mario Exp1 CDF (decal vs matched pure-streaming baseline)",
        "",
