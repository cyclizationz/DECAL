...
from common import EVAL_DIR, ensure_dir, load_manifest
...
GAME_MARKERS = {"fc5": "o", "fm6": "s", "mario": "o", "spaceflight": "D"}
GAME_LABELS = {"fc5": "FC5", "fm6": "FM6", "mario": "Mario", "spaceflight": "SC"}
GAME_COLORS = {"fc5": "tab:blue", "fm6": "tab:green", "mario": "#E6B800", "spaceflight": "red"}
...
def generality_scatter_plot(
    ...
    *,
    ...
    for row in plot_rows:
        ...
        color = GAME_COLORS[game] if game == "spaceflight" else STYLE_COLORS.get(style, "tab:gray")
...
def write_ref_ratio_table(rows: list[dict[str, Any]], out_dir: Path) -> None:
    ...
    for game_key in sorted(by_game):
        ...
        if game_key == "spaceflight":
            tex_lines.append(
                rf"\textcolor{{red}}{{{game}}} & \textcolor{{red}}{{{ref_r:.3f}}} & "
                rf"\textcolor{{red}}{{{bsp:.1f}}} & \textcolor{{red}}{{{area:.1f}}} & "
                rf"\textcolor{{red}}{{{reuse:.3f}}} \\"
            )
        else:
            tex_lines.append(
                f"{game} & {ref_r:.3f} & {bsp:.1f} & {area:.1f} & {reuse:.3f} \\\\"
...
def main() -> None:
    ...
    ap.add_argument("--rd-points", type=Path, default=EVAL_DIR / "exp1" / "rd_suite_points.csv")
    ...
    ap.add_argument("--out-dir", type=Path, default=EVAL_DIR / "exp5")
    args = ap.parse_args()

    ensure_dir(args.out_dir)
    manifest = {clip.clip_id: clip for clip in load_manifest(args.manifest)}
    for manifest_path in args.extra_manifest:
        manifest.update({clip.clip_id: clip for clip in load_manifest(manifest_path)})
    rows: list[dict[str, Any]] = []
    ...
    for row in rd_rows:
            ...
            decal_bps = float(row.get("decal_achieved_bps", 0.0) or 0.0)
            bsp = (1.0 - decal_bps / baseline_bps) * 100.0 if baseline_bps > 1e-9 else 0.0
            rows.append(
                {
                    ...
                    "decal_vmaf_mean": float(row.get("vmaf_mean", 0.0) or 0.0),
    ...
    if rows:
        ...
        summary_lines = [
            ...
            "",
            "This summary is intentionally conservative. It is meant to support the claim that DECAL helps most when recurring objects are both large and reusable, and that it degrades gracefully when those conditions fail.",
            "",
            ...
            "",
            '- "DECAL is strongest when recurring objects are large and reusable; when these conditions fail, the system gracefully falls back and rarely harms quality."',
            "",
