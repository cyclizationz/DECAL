...
REPO_ROOT = _env_path("DECAL_ROOT", Path(__file__).resolve().parents[2])
DATA_ROOT = _env_path("DECAL_DATA", REPO_ROOT / "data")
EVAL_DIR = _env_path("DECAL_EVAL", REPO_ROOT / "eval")
VIDEO_DIR = DATA_ROOT / "clips"
RECORD_DIR = EVAL_DIR
DATASETS_ROOT = DATA_ROOT / "sources"
DEPLOYMENT_BUILD_DIR = REPO_ROOT / "deployment" / "build"
OFFLINE_BIN = DEPLOYMENT_BUILD_DIR / "decal_offline"
CURRENT_BASELINE_BIN = DEPLOYMENT_BUILD_DIR / "decal_baseline"
DUO_BIN = DEPLOYMENT_BUILD_DIR / "decal_duo"
SERVER_BIN = DEPLOYMENT_BUILD_DIR / "decal_server"
CLIENT_BIN = DEPLOYMENT_BUILD_DIR / "decal_client"
TEMPLATE_SERVER_BIN = DEPLOYMENT_BUILD_DIR / "decal_template_server"
DEFAULT_MODEL = Path(os.environ.get("DECAL_MODEL", str(DATA_ROOT / "models" / "seg.onnx")))
FC5_MODEL = DATA_ROOT / "models" / "fc5_seg.onnx"
FM6_MODEL = DATA_ROOT / "models" / "fm6_seg.onnx"
SPACEFLIGHT_MODEL = DATA_ROOT / "models" / "spaceflight_seg.onnx"
FC5_LATENT_BANK = DATA_ROOT / "banks" / "fc5" / "latent_bank.json"
FM6_LATENT_BANK = DATA_ROOT / "banks" / "fm6" / "latent_bank.json"
SPACEFLIGHT_LATENT_BANK = DATA_ROOT / "banks" / "spaceflight" / "latent_bank.json"
MARIO_TEMPLATES = DATA_ROOT / "templates" / "mario"
DEFAULT_THREADS = 1
...
def python_bin() -> str:
    env = os.environ.get("DECAL_PYTHON")
    if env:
        return env
...
def load_manifest(path: Path) -> list[ClipSpec]:
    ...
    clips: list[ClipSpec] = []
    for row in doc:
        for key in ("source_path", "clip_path", "normalized_path"):
            if key in row and row[key]:
                row[key] = _resolve_data_path(str(row[key]))
        clips.append(ClipSpec(**row))
    return clips
