...
static void print_usage(const char *program_name) {
    std::cout << "Usage: " << program_name << " [OPTIONS]\n"
              << "Options:\n"
              << "  -i <input_video_path>   Input video file (required)\n"
              << "  -o <output_dir>         Output directory for comparison artifacts (default: outputs/duo)\n"
              << "  -m <model_path>         Path to the ONNX model (default: models/seg.onnx)\n"
...
int main(int argc, char *argv[]) {
    ...
    OfflineOptions opt;
    opt.source = ""; // required via -i/--input
    opt.model = "models/seg.onnx";
    opt.outDir = "outputs/duo";
    ...
    if (opt.source.empty()) {
        std::cerr << "Missing required -i <input_video_path>\n";
        print_usage(argv[0]);
        return 2;
    }
