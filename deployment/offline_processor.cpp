...
static std::string choose_templates_dir(const OfflineOptions &opt) {
    ...
    return std::string("."); // set -T or TM_TEMPLATE_PATH for pixel mode
}
...
static bool template_alpha_mask_metrics(
    const cv::Mat &tpl_rgba_resized,
    const cv::Mat &full_mask_u8,
    const cv::Rect &bbox,
    double &iou_out,
    double &extra_ratio_out
) {
    iou_out = 0.0;
    extra_ratio_out = 1.0;
    if (tpl_rgba_resized.empty() || full_mask_u8.empty()) return false;
    ...
    if (safe.area() <= 0) return false;
    if (tpl_rgba_resized.cols != bbox.width || tpl_rgba_resized.rows != bbox.height) return false;
    uint64_t inter = 0, uni = 0, extra = 0;
    int dx0 = safe.x - bbox.x;
    int dy0 = safe.y - bbox.y;
    for (int y = 0; y < safe.height; ++y) {
        int my = safe.y + y;
    ...
    double t_area = (double)(inter + extra);
    if (t_area <= 0.0) return false;
    iou_out = (double)inter / (double)uni;
    extra_ratio_out = (double)extra / t_area;
    return true;
}
...
int run_offline_evaluation(const OfflineOptions &opt, YOLO_V8& yoloDetector) {
    ...
    fs::path gate_debug_crop_dir;
    std::ofstream gate_debug_csv;
    int gate_debug_counter = 0;
    if (!opt.gateDebugDir.empty()) {
        fs::create_directories(opt.gateDebugDir);
        gate_debug_crop_dir = fs::path(opt.gateDebugDir) / "crops";
        fs::create_directories(gate_debug_crop_dir);
        gate_debug_csv.open((fs::path(opt.gateDebugDir) / "gate_debug.csv").string());
        gate_debug_csv << "frame,det_idx,template_id,stage,pass,metric,threshold,x,y,w,h,crop_path\n";
    }
            ...
            if (opt.pixelMode) {
                ...
                if (yolo_matcher == "latent_key") {
                    ...
                    if (opt.yoloHealOnly) {
                        ...
                        auto write_gate_debug = [&](const std::string &stage,
                                                    bool passed,
                                                    double metric,
                                                    const std::string &threshold,
                                                    uint32_t cand_id,
                                                    const cv::Rect &box) {
                            if (!gate_debug_csv.is_open()) return;
                            cv::Rect frame_rect(0, 0, frame.cols, frame.rows);
                            cv::Rect safe_box = box & frame_rect;
                            if (safe_box.area() <= 0) return;
                            const int crop_w = std::min(640, frame.cols);
                            const int crop_h = std::min(360, frame.rows);
                            int cx = safe_box.x + safe_box.width / 2;
                            int cy = safe_box.y + safe_box.height / 2;
                            int x0 = std::max(0, std::min(frame.cols - crop_w, cx - crop_w / 2));
                            int y0 = std::max(0, std::min(frame.rows - crop_h, cy - crop_h / 2));
                            cv::Rect crop_rect(x0, y0, crop_w, crop_h);
                            cv::Mat crop = frame(crop_rect).clone();
                            cv::Rect rel_box(safe_box.x - x0, safe_box.y - y0, safe_box.width, safe_box.height);
                            cv::rectangle(crop, rel_box, passed ? cv::Scalar(0, 180, 0) : cv::Scalar(0, 0, 220), 2);
                            std::ostringstream name;
                            name << "gate_" << std::setw(5) << std::setfill('0') << gate_debug_counter++
                                 << "_f" << frameIdx << "_d" << di << "_" << stage
                                 << "_" << (passed ? "pass" : "fail") << ".png";
                            fs::path crop_path = gate_debug_crop_dir / name.str();
                            try { cv::imwrite(crop_path.string(), crop); } catch (...) {}
                            gate_debug_csv << frameIdx << "," << di << "," << cand_id << ","
                                           << stage << "," << (passed ? 1 : 0) << ","
                                           << std::fixed << std::setprecision(6) << metric << ","
                                           << threshold << "," << safe_box.x << "," << safe_box.y << ","
                                           << safe_box.width << "," << safe_box.height << ","
                                           << crop_path.string() << "\n";
                        };
                        ...
                        auto try_id = [&](uint32_t cand_id) -> bool {
                            ...
                            if (itIdx == latent_id_to_index.end()) {
                                write_gate_debug("resident", false, 0.0, "1", cand_id, d.box);
                                return false;
                            }
                            write_gate_debug("resident", true, 1.0, "1", cand_id, d.box);
                            ...
                            write_gate_debug("appearance_mse", mse <= (double)opt.yoloHealAppearanceMseThr, mse,
                                             std::to_string(opt.yoloHealAppearanceMseThr), cand_id, d.box);
                            ...
                            write_gate_debug("coverage", cov >= (double)opt.yoloHealMinMaskCoverage, cov,
                                             std::to_string(opt.yoloHealMinMaskCoverage), cand_id, d.box);
                            if (cov < (double)opt.yoloHealMinMaskCoverage) return false;
                            stitch_box = d.box + cv::Point(dx, dy);
                            double geom_iou = 0.0, geom_spill = 1.0;
                            (void)template_alpha_mask_metrics(img, d.boxMask, stitch_box, geom_iou, geom_spill);
                            bool geom_ok = template_alpha_matches_mask(img, d.boxMask, stitch_box,
                                                                       opt.yoloHealMaskIouThr,
                                                                       opt.yoloHealMaskExtraThr);
                            {
                                std::ostringstream thr;
                                thr << "iou>=" << opt.yoloHealMaskIouThr << ";spill<=" << opt.yoloHealMaskExtraThr;
                                // Store the stricter value first for selection; the threshold field carries both tests.
                                write_gate_debug("geometry_iou", geom_ok, geom_iou, thr.str(), cand_id, stitch_box);
                                write_gate_debug("geometry_spill", geom_ok, geom_spill, thr.str(), cand_id, stitch_box);
                            }
                            if (!geom_ok) {
                                return false;
                            }
                        ...
                        if (chosen_id == 0 || force_mint) {
                            write_gate_debug("resident", false, 0.0, "1", chosen_id, d.box);
                        }
