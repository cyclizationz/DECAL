...
void print_usage(const char *program_name) {
    std::cout
        ...
        << "\n"
        << "DECAL online-server scaffold. This wraps the current evaluator in\n"
        ...
        << "      --yolo-heal-iou <f>     Gate geometry IoU threshold\n"
        << "      --yolo-heal-extra <f>   Gate alpha spill threshold\n"
        << "      --yolo-heal-coverage <f> Gate mask coverage threshold\n"
        << "      --yolo-heal-mse <f>     Gate appearance MSE threshold\n"
        ...
        << "      --gate-debug-dir <dir> Export reliability-gate metric crops\n"
...
int main(int argc, char **argv) {
    OfflineOptions opt;
    opt.source = ""; // required via --input
    opt.model = "models/seg.onnx";
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (arg == "--yolo-heal-iou") {
            if (!require_value(i, argc, argv, value)) return 2;
            opt.yoloHealMaskIouThr = std::stof(value);
        } else if (arg == "--yolo-heal-extra") {
            if (!require_value(i, argc, argv, value)) return 2;
            opt.yoloHealMaskExtraThr = std::stof(value);
        } else if (arg == "--yolo-heal-coverage") {
            if (!require_value(i, argc, argv, value)) return 2;
            opt.yoloHealMinMaskCoverage = std::stof(value);
        } else if (arg == "--yolo-heal-mse") {
            if (!require_value(i, argc, argv, value)) return 2;
            opt.yoloHealAppearanceMseThr = std::stof(value);
        ...
        } else if (arg == "--gate-debug-dir") {
            if (!require_value(i, argc, argv, opt.gateDebugDir)) return 2;
    ...
    if (opt.source.empty()) {
        std::cerr << "Missing required --input <video>\n";
        print_usage(argv[0]);
        return 2;
    }
